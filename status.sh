...
Download_Server_Status_server(){
	cd "/tmp"
	wget -N --no-check-certificate "https://github.com/dbconf/ServerStatus-Hotaru/archive/master.zip"
...
Download_Server_Status_client(){
	cd "/tmp"
	wget -N --no-check-certificate "https://github.com/dbconf/ServerStatus-Hotaru/raw/master/clients/status-client.py"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://github.com/dbconf/ServerStatus-Hotaru/raw/master/service/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "github.com/dbconf/ServerStatus-Hotaru/raw/master/service/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "github.com/dbconf/ServerStatus-Hotaru/raw/master/service/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "github.com/dbconf/ServerStatus-Hotaru/raw/master/service/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
...
默认为本机IP为域名，例如输入: dabai.in ，如果要使用本机IP，请留空直接回车"
		read -e -p "(默认: 本机IP):" server_s
		[[ -z "$server_s" ]] && server_s=""
	else
		echo -e "请输入 ServerStatus 服务端的 IP/域名[server]"
		read -e -p "(默认: s.dabai.in):" server_s
		[[ -z "$server_s" ]] && server_s="ss.dabai.in"
	fi
...
Set_server_port(){
	while true
		...
		read -e -p "(默认: 33389):" server_port_s
		[[ -z "$server_port_s" ]] && server_port_s="33389"
...
Set_password(){
	...
	read -e -p "(默认: dabai):" password_s
	[[ -z "$password_s" ]] && password_s="dabai"
	echo && echo "	================================================"
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		...
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/dbconf/ServerStatus-Hotaru/caddy/caddy_install.sh
...
Uninstall_ServerStatus_server(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		if [[ -e "/etc/init.d/caddy" ]]; then
			...
			wget -N --no-check-certificate https://github.com/dbconf/ServerStatus-Hotaru/raw/master/caddy/caddy_install.sh
			chmod +x caddy_install.sh
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://www.dabai.in/sh/status.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	wget -N --no-check-certificate "https://www.dabai.in/sh/status.sh" && chmod +x status.sh
